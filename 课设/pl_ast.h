...
std::vector<ast> &ast::getchild() { return child; }
int ast::getline() { return line; }
int ast::gettype() { return type; }
std::string ast::getstr() { return content; }
...
void ast::print(int depth = 0) {
  ...
  if (type == ast_id || type == ast_number || type == ast_prog ||
      type == ast_proc || type == ast_call || type == ast_assign)
    std::cout << ": " << content;
  std::cout << "\n";
