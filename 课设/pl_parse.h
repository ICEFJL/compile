...
void panic() {
  while (token.tok_type != tok_semi && token.tok_type != tok_begin &&
         token.tok_type != tok_end && token.tok_type != tok_do &&
         token.tok_type != tok_then) {
    next();
...
ast lexp() {
  ...
  if (token.tok_type == tok_odd) {
    ...
    switch (node.gettype()) {
    case ast_eq:
      emit(op_opr, 0, calc_eq);
      break;
    case ast_neq:
      emit(op_opr, 0, calc_neq);
      break;
    case ast_less:
      emit(op_opr, 0, calc_les);
      break;
    case ast_leq:
      emit(op_opr, 0, calc_leq);
      break;
    case ast_great:
      emit(op_opr, 0, calc_grt);
      break;
    case ast_geq:
      emit(op_opr, 0, calc_geq);
      break;
    }
    node.addchild(tmp1);
    node.addchild(tmp2);
...
ast lop() {
  ast node(line);
  if (token.tok_type == tok_equal || token.tok_type == tok_neq ||
      token.tok_type == tok_less || token.tok_type == tok_leq ||
      token.tok_type == tok_great || token.tok_type == tok_geq) {
    int atype;
