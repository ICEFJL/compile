...
const char *opcode_name[] = {"nop", "lit", "opr", "lod", "sto", "cal",
                             "int", "jmp", "jpc", "red", "wrt", NULL};
enum opr_type {
  calc_ret,
  calc_nega,
  calc_plus,
...
void emit(unsigned char opcode = op_nop, unsigned char level = 0,
          int opnum = 0) {
...
void add_new_symbol(int type = sym_var, int const_number = 0,
...
void set_procedure_arg_size(std::string symbol_name, int arg_size) {
  for (std::list<std::vector<symbol>>::iterator i = symbol_table.begin();
